...
export function ProductGallery({ media, productTitle }: ProductGalleryProps) {
  ...
  const active = media[Math.min(index, count - 1)];
  ...
  return (
    <div className="flex min-w-0 flex-col-reverse gap-4 lg:flex-row">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          {count > VISIBLE_THUMBS ? (
            <button
              ...
              onClick={() => railRef.current?.scrollBy({ top: 108, behavior: "smooth" })}
              className={cn(floatingButton, "hidden lg:flex")}
            >
              <CaretDownIcon aria-hidden="true" size={ICON_SIZE_SM} weight={ICON_WEIGHT_OUTLINE} />
            </button>
          ) : null}
        </div>
      ) : null}
      ...
      <div className="relative min-w-0 flex-1">
        ...
        >
          <CornersOutIcon aria-hidden="true" size={ICON_SIZE_SM} weight={ICON_WEIGHT_OUTLINE} />
        </button>
      </div>
          ...
          {count > 1 ? (
            <>
              <button
                ...
                className={cn(floatingButton, "absolute left-4 top-1/2 -translate-y-1/2")}
              >
              ...
              <button
                ...
                onClick={() => show(index + 1)}
                className={cn(floatingButton, "absolute right-4 top-1/2 -translate-y-1/2")}
              >
